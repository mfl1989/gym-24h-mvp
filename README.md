...
后端、前端、Stripe CLI webhook listener を一括で起動する場合は、以下を実行します。

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\start-local-dev.ps1 -OpenBrowser -StartStripeListener
```

起動した本地开发相关进程を一括で停止する場合は、以下を実行します。
...
この bat は local front/back/Stripe listener を起動した後に、ngrok と Cloudflare の窗口も開きます。
